...
class CreateGameProcessesTable extends Migration {
    ...
    public function up()
    {
        Schema::create('game_processes', function(Blueprint $table)
        {
            ...
            $table->string('os_version', 256)->comment('系统');
            ...
            // publish =>上传 pending =>待审核 notpass=>不通过 draft => 草稿 unstock => 下架
            $table->enum('status', ['publish', 'pending', 'notpass', 'draft', 'unstock'])->default('publish')->nullable()->comment('状态');
