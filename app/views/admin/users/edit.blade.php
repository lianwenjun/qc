...
<div class="Content_right_top Content_height">
    <div class="Theme_title">
        <h1>管理中心 <span> 管理员管理 </span> <b> 编辑管理员 </b></h1>
    ...
    @endif
    <!-- /提示 -->
    <form action="{{ URL::route('users.edit', ['id' => $user['id']]) }}" id="form" method="POST">
    ...
    <table  border="0" cellspacing="0" cellpadding="0" class="add_Activation">
      <tr>
        ...
        <td>
           <select id="u1884_input" tabindex="0" name="group_id">
                ...
                <option value="{{ $role->id }}" @if(in_array($role->id, $groupIds))selected="selected"@endif>{{ $role->name }}</option>
                @endforeach
            </select>
        </td>
      </tr>
      <tr>
        ...
        <td>
          <span><input name="activated" type="radio" value="1" @if($user['activated'] == 1)checked="checked"@endif/>正常</span>
          <span><input name="activated" type="radio" value="0" @if($user['activated'] == 0)checked="checked"@endif/>禁用</span>
        </td>
      </tr>
       <tr>
        <td align="right">用户名：</td>
        <td><input name="username" type="text" class="add_Activation_text" value="{{ $user['username'] }}"/></td>
      </tr>
      <tr>
        <td align="right">登陆密码：</td>
        <td><input name="password" type="password" id="password" class="add_Activation_text"  placeholder="不修改请留空"/></td>
      </tr>
      <tr>
      ...
      <tr>
        ...
        <td><input name="realname" type="text" class="add_Activation_text" value="{{ $user['realname'] }}"/></td>
      </tr>
      <tr>
        <td align="right">邮箱：</td>
        <td><input name="email" type="text" class="add_Activation_text" value="{{ $user['email'] }}"/></td>
      </tr>
    </table>
    ...
    $('.jq-submit').click(function() {
        ...
        $("#form").validate({
            ...
            messages: {
                ...
                password: {maxlength: '密码不能大于15个字符'},
