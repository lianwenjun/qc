...
<div class="breadcrumb"><a href="javascript: ;">广告位管理</a><span>&gt;</span>首页游戏位管理</div>  
    <div class="content"> 
        ...
        <p class="record-title">上架游戏：共
            <b class="red">{{ $datas['apps']->getTotal()}}</b>条记录
            @if(Sentry::getUser()->hasAccess('ads.apps.create'))
            <a href="{{ URL::route('ads.apps.create') }}" class="button fr">新增推广</a>
            @endif
        </p>
        <table cellpadding="0" cellspacing="0" class="zebra-table tc">
            ...
            <tbody>
                @forelse($datas['apps'] as $app)
                <tr>
                    <td>{{ $app->id }}</td>
                    <td><img src="../images/pages/icon1.jpg" alt="图标1" class="icon25" /></td>
                    <td><span class="elimit6em cursor" title="我叫MT  online">{{ $app->title }}</span></td>
                    <td>{{ $datas['locations'][$app->location] }}</td>
                    <td>{{ $app->sort }}</td>
                    <td><span class="elimit8em cursor" title="{{ date('Y-m-d h:m:s', $app->stocked_at) }}">{{ date('Y-m-d h:m:s', $app->stocked_at) }}</span></td>
                    <td><span class="elimit8em cursor" title="{{ date('Y-m-d h:m:s', $app->unstocked_at) }}">{{ date('Y-m-d h:m:s', $app->unstocked_at) }}</span></td>
                    <td class="green">{{ $datas['status'][$app->status] }}</td>
                    <td>{{ $app->operator }}</td>
                    <td>
                        @if(Sentry::getUser()->hasAccess('ads.apps.unstock'))
                        <a href="javascript:;" class="button jq-unstock @if($app->status != 'stock') disabled @endif" data-url="@if($app->status == 'stock'){{ URL::route('ads.apps.unstock', $app->id) }} @endif">下架</a>
                        @endif
                        @if(Sentry::getUser()->hasAccess('ads.apps.edit'))
                        <a href="@if($app->status == 'stock'){{ URL::route('ads.apps.edit') }} @else javascript:; @endif" class="button @if($app->status != 'stock') disabled @endif">编辑</a>
                        @endif
                        @if(Sentry::getUser()->hasAccess('ads.apps.delete'))
                        <a href="javascript:;" class="@if($app->status == 'stock') button disabled @else red-button @endif" data-url="@if($app->status != 'stock') {{URL::route('ads.apps.delete', $app->id)}} @endif">删除</a>
                        @endif
                    </td>               
                </tr>
                @empty
